...
function buildRanges(lengths, viewport) {
  const order = ['land', 'drift', 'gather', 'burst', 'astro', 'nova', 'dust', 'grid', 'work'];
...
check('track total (vh)', total, 2465);
check('scrollable travel (vh)', travel, 2365);
...
check('work.startVh', ranges.work.startVh, 1415);
...
check('B4 (work start as fraction)', B4, 1415 / 2365, 0);
...
check('grid.end === work.start', ranges.grid.endVh, ranges.work.startVh);
